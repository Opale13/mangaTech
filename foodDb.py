    ...
    def addStore (self, tom_title, store_name, purchase_date):
        ...
        try:
            ...
            if (result.single() == None): 
                try:
                    #Tom-[PURCHASED]->Store
                    request = "MATCH (s:Store) WHERE s.name = '{}' MATCH (t:Tom) WHERE t.title = '{}' SET t.possessed=True CREATE (t) -[r:PURCHASED]-> (s) SET r.purchaseDate='{}'".format(store_name, tom_title, purchase_date)
                    session = self._driver.session()
                    session.run(request)              
                    session.close()
                except:
